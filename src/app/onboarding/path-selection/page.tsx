...
export default function PathSelectionPage() {
  ...
  const handleNext = (data: any) => {
    ...
    // Handle redirect to entities page from path selection
    if (data.redirectToEntities) {
      const storeType = data.storePath === 'existing' ? 'existing' : 'new';
      window.location.href = `/entities?storeType=${storeType}`;
      return;
    }
  };
  ...
  return (
    <>
        ...
        <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              ...
              <button
                onClick={() => window.location.href = '/entities?storeType=new'}
                className="bg-yellow-200 text-yellow-800 px-3 py-1 rounded text-sm font-medium hover:bg-yellow-300 transition-colors"
              >
                Next →
              </button>
      ...
      <div className="max-w-4xl mx-auto px-6 pb-20">
        <PathSelection onNext={handleNext} onBack={handleBack} userData={userData} />
      </div>
    </>
  );
}